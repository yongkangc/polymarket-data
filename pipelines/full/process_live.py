...
def process_live():
    ...
    print(f"🔍 Looking for last processed transaction...")
    print(f"   Timestamp: {last_processed['timestamp']}")
    print(f"   Hash: {last_processed['transactionHash']}")
    print(f"   Found {len(same_timestamp)} matching rows")

    if len(same_timestamp) == 0:
        print("⚠️  Could not find last processed transaction - processing all rows")
